...
var tree, setting =
{
	...
	callback :
	{
		beforeClick : function(treeId, treeNode)
		{
			if ("${checked}" == "true")
			{
				// tree.checkNode(treeNode, !node.checked, true,
				// true);
		...
		onCheck : function(e, treeId, treeNode)
		{
			...
			for (var i = 0, l = nodes.length; i < l; i++)
			{
				...
				setTitleTs(event, treeId, treeNode);
			}
			return false;
		},
		onAsyncSuccess : function(event, treeId, treeNode, msg)
		{
			...
			for (var i = 0, l = nodes.length; i < l; i++)
			{
				setTitleTs(event, treeId, nodes[i]);
				try
				{
		...
		{// <c:if test="${!checked}">
			...
			// $("input[type='text']",
			// top.mainFrame.document).focus();//</c:if>
		}
	}
};
...
function showAllNode(nodes)
{
	...
	{
		/*
		 * if(!nodes[i].isParent){ tree.showNode(nodes[i]); }else{
		 */
